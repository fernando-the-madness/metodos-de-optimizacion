...
def generate_sample_data(start_date: str, periods: int, pattern: str = 'sinusoidal') -> pd.DataFrame:
    """
    Genera datos de ejemplo para series temporales.
    
    Parámetros:
    - start_date: Fecha inicial en formato 'YYYY-MM-DD'
    - periods: Número de períodos a generar
    - pattern: Tipo de patrón ('sinusoidal', 'linear', 'random')
    """
    dates = pd.date_range(start=start_date, periods=periods, freq='D')
    
    if pattern == 'sinusoidal':
        # Genera un patrón sinusoidal con ruido
        t = np.linspace(0, 10, periods)
        values = 100 + 20 * np.sin(t) + 5 * np.random.randn(periods)
    elif pattern == 'linear':
        # Genera una tendencia lineal con ruido
        values = np.linspace(50, 150, periods) + 10 * np.random.randn(periods)
    else:  # random
        # Genera un camino aleatorio
        values = 100 + np.random.randn(periods).cumsum()
...
# Modificación del código principal para incluir la opción de generar datos
def main():
    st.title('Predicción de Series Temporales con Interpolación Fractal')
    st.sidebar.header('Configuración')
    
    # Opción para usar datos de ejemplo o subir archivo
    data_option = st.sidebar.radio(
        "Selecciona la fuente de datos:",
        ["Generar datos de ejemplo", "Subir archivo CSV"]
    )
    
    if data_option == "Generar datos de ejemplo":
        st.sidebar.subheader("Configuración de datos de ejemplo")
        ...
        data = generate_sample_data(start_date.strftime('%Y-%m-%d'), periods, pattern)
        
        # Opción para descargar los datos generados
        csv = data.to_csv()
        st.sidebar.download_button(
            label="Descargar datos generados",
            data=csv,
        ...
        if not uploaded_file:
            st.warning('Por favor sube un archivo CSV o selecciona "Generar datos de ejemplo"')
            st.info('El archivo CSV debe tener el siguiente formato:')
            st.code("""fecha,valor
2023-01-01,100.5
2023-01-02,101.2
2023-01-03,99.8
